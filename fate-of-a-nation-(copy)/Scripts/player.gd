...
func apply_powerup(powerup_type: int, amount: int = 1, duration: float = 6.0) -> void:
	if powerup_type == Powerup.PowerupType.HEALTH:
		_heal(amount)
		return

	if powerup_type == Powerup.PowerupType.RAPID_FIRE:
		_activate_rapid_fire(duration)
		return

	if powerup_type == Powerup.PowerupType.SPREAD_SHOT:
		_activate_spread_shot(duration)
		return
	...
	if powerup_type == Powerup.PowerupType.BOMB:
		_activate_bomb()
		return
