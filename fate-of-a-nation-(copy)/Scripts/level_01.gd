...
@export var tank_spawn_interval: int = 50
...
var tank_warning_active: bool = false
...
func _spawn_player() -> void:
	if player_scene == null:
		push_warning("Level01: player_scene is not assigned.")
		return
	...
	if player == null:
		push_warning("Level01: spawned player is not a Player.")
		return
...
func spawn_enemy() -> void:
	...
	if enemy_basic_scene == null:
		push_warning("Level01: enemy_basic_scene is not assigned.")
		return
	...
	if enemies_spawned % tank_spawn_interval == 0:
		_start_tank_warning()

	_update_spawn_speed()


func _start_tank_warning() -> void:
	if tank_warning_active:
		return

	if tank_enemy == null:
		return

	tank_warning_active = true

	var tank_spawn_position: Vector2 = _get_random_spawn_position()

	if tank_warning_scene != null:
		var warning: Node = tank_warning_scene.instantiate()
		add_child(warning)
		warning.global_position = tank_spawn_position

		shake_camera(0.35)

		if warning.has_signal("finished"):
	...
	if GameState.is_game_over:
		tank_warning_active = false
		return

	_spawn_tank_at_position(tank_spawn_position)
	tank_warning_active = false


func _spawn_tank_at_position(spawn_position: Vector2) -> void:
	if tank_enemy == null:
		return

	var tank: Node = tank_enemy.instantiate()
	enemy_container.add_child(tank)
	tank.global_position = spawn_position

	shake_camera(0.65)
	print("TANK SPAWNED at count:", enemies_spawned)
...
func spawn_powerup() -> void:
	if powerup_scene == null:
		push_warning("Level01: powerup_scene is not assigned.")
		return
...
func shake_camera(strength: float = 0.35) -> void:
	...
	if camera_shake == null:
		print("Level01: No CameraShake found.")
		return
